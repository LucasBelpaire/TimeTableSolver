...
def construct_time_table():
    ...
    while (len(gv.unplaced_events) > 0 or len(gv.events) > 0) \
            and time.clock() - start_construct < gv.initial_construct_time:

        # the events get sorted, harder to place events will get placed first
        sorted_events = order_course_events_by_priority(gv.events, gv.courses_set)
        ...
        for index, course_event in enumerate(sorted_events):
            ...
            if len(available_positions) == 0:
                gv.unplaced_events.append(course_event)
                continue
            ...
            gv.assign_course_to_position(course_event, best_fit)


def order_course_events_by_priority(course_events, courses_set):
    """
    ...
    lectures_amount = count_lectures_per_course(courses_set)
    ...
    for course_event in course_events:
        ...
        course_events_ranking[course_event.course_code].append(get_events_ranking2(course_event, list(courses_set)))
